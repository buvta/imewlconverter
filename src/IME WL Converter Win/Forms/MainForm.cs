...
public partial class MainForm : Form
{
    ...
    private readonly SortType sortType = SortType.Default;
    ...
    private void btnConvert_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            mainBody.SelectedTranslate = translate;
            mainBody.SelectedConverter = chineseConverter;
            mainBody.Filters = GetFilters();
            mainBody.SortType = sortType;
            mainBody.SortDesc = sortDesc;
    ...
    private IList<IReplaceFilter> GetReplaceFilters()
    {
        ...
        if (filterConfig.ReplaceEnglish) filters.Add(new EnglishFilter());
        ...
        if (filterConfig.ReplaceSpace) filters.Add(new SpaceFilter());
        if (filterConfig.ReplaceNumber) filters.Add(new NumberFilter());
        if (cbxFrom.Text == ConstantString.EMOJI) filters.Add(new EmojiReplacer(txbWLPath.Text));
        return filters;
    }
    ...
    private IList<ISingleFilter> GetFilters()
    {
        ...
        if (filterConfig.WordLengthFrom > 1 || filterConfig.WordLengthTo < 9999) filters.Add(lenFilter);
        var rankFilter = new RankFilter();
        rankFilter.MaxLength = filterConfig.WordRankTo;
        rankFilter.MinLength = filterConfig.WordRankFrom;
        if (filterConfig.WordRankFrom > 1 || filterConfig.WordRankTo < 999999) filters.Add(rankFilter);
    ...
    private void LoadImeList()
    {
        ...
        cbxFrom.Items.Clear();
        foreach (var comboBoxShowAttribute in cbxImportItems) cbxFrom.Items.Add(comboBoxShowAttribute.Name);
        cbxTo.Items.Clear();
        foreach (var comboBoxShowAttribute in cbxExportItems) cbxTo.Items.Add(comboBoxShowAttribute.Name);
    }
    ...
    private IChineseConverter chineseConverter;
    ...
    private void ToolStripMenuItemRankGenerate_Click(object sender, EventArgs e)
    {
        ...
        if (form.ShowDialog() == DialogResult.OK) wordRankGenerater = form.SelectedWordRankGenerater;
    }
    ...
    private void timer1_Tick(object sender, EventArgs e)
    {
        if (mainBody != null)
            try
            {
                ...
                var count = mainBody.CountWord;
                var current = mainBody.CurrentStatus;
                //ShowStatusMessage("转换进度：" + mainBody.CurrentStatus + "/" + mainBody.CountWord, false);
                toolStripProgressBar1.Maximum = count;
                if (count > 0 && current <= count) toolStripProgressBar1.Value = current;
            }
            catch (Exception ex)
            {
