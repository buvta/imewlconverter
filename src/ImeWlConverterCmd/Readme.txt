注意：该小工具基于C#开发，需要.NET 10.0运行时环境，支持Windows、Linux和macOS平台。如果双击"深蓝词库转换.exe"后弹出错误窗口，请下载安装.NET 10.0再试。
...
1.8版增加了自定义编码的输出，增强了命令行功能，请使用-h查看命令行帮助。实现了百度手机分类词库（bcd格式）、小小输入法和微软拼音输入法的词库功能，但是可能由于输入法的原因，会导入失败。
...
3.3版增加了macOS GUI应用（基于Avalonia UI），修复了自定义编码功能、没有词频的Rime拼音码表、新世纪五笔生成器等问题，改进了拼音规则，支持CI交叉编译为单文件。
3.3.1版修复了WinForm构建错误，实现了版本号自动化生成机制，修复了Rime导出和拼音转换出错的问题。
3.4版升级.NET到10.0，支持导出搜狗细胞词库scel格式，增加了LLM词频生成功能，重构了命令行参数为GNU风格，增加了集成测试框架，修复了GBoard输出格式、Mac系统编码支持、Unicode扩展区字符处理、多音字标注逻辑、大文件内存溢出、汉字〇被误判为标点等多个问题。
